# Purpose: measure and plot fin rays for specimen Eusthenopteron CMNH8153
#
# note: 
...
eusthenopteron_all_data <- read.table("data_raw/CMNH8153_CSA_and_SMA.txt", head=TRUE)
...
d1_vs_v1_mom <- as.vector(c("Eusthenopteron", "CMNH8153", "d1_vs_v1", "sma", wilcox.test(eusth_d1$second_moment_min, eusth_v1$second_moment_min)$statistic, wilcox.test(eusth_d1$second_moment_min, eusth_v1$second_moment_min)$p.value))
d2_vs_v2_mom <- as.vector(c("Eusthenopteron", "CMNH8153", "d2_vs_v2", "sma", wilcox.test(eusth_d2$second_moment_min, eusth_v2$second_moment_min)$statistic, wilcox.test(eusth_d2$second_moment_min, eusth_v2$second_moment_min)$p.value))
d3_vs_v3_mom <- as.vector(c("Eusthenopteron", "CMNH8153", "d3_vs_v3", "sma", wilcox.test(eusth_d3$second_moment_min, eusth_v3$second_moment_min)$statistic, wilcox.test(eusth_d3$second_moment_min, eusth_v3$second_moment_min)$p.value))
